// Dafny program test4.dfy compiled into C#
...
// Command Line Options: c:\Users\Tardi\OneDrive\文档\Dafny-Programs\test4.dfy /verifyAllModules /compile:1 /spillTargetCode:1 /out:bin\test4
// test4.dfy
...
function Join(s: seq<string>, d: char): (output: string)
  requires |s| > 0
  requires forall i: seq<char> | i in s :: d !in i
  ensures |output| >= 0
  ensures s[0] <= output
  decreases s, d
{
...
function split(s: string, d: char): (output: seq<string>)
  ...
  decreases s, d
{
  if d !in s then
    ...
    ghost var index: nat := FindNextDelimiter(s, d, 0); [s[..index]] + split(s[index + 1..], d)
}
")]
